...
class DashOrthoParser():
    ...
    def get_HOG_protein_seqs(self, HOG):
        """Return df with proteinID as index, accession, HOG and protein sequence """
        prot_names = self.all_prots_in_HOG(HOG)
        d = {}
        for prot_name in prot_names:
            acc= prot_name[:15][::-1].replace('_', '.',1)[::-1]
            prot_id = prot_name[16:]
            path_to_fasta = f'./data/Proteomes/{acc}.faa'
            seq = pfa.get_seq_dict(path_to_fasta).get(prot_id)
            d[prot_name] = [acc, HOG, seq]
        df = pd.DataFrame.from_dict(d, orient='index')
        df.columns = ['Accession', 'HOG', 'Protein_seq']
        return df
